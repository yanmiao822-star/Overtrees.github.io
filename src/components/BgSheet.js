      ...
      R.createElement('div', { style:{ position:'fixed', left:0, right:0, bottom:'calc(env(safe-area-inset-bottom) + 14px)', zIndex:4001, display:'flex', justifyContent:'center', padding:'0 14px', pointerEvents:'none', opacity:closing?0:1, transform:closing?'translateY(20px)':'translateY(0)', transition:'opacity 200ms ease, transform 250ms cubic-bezier(0.34,1.56,0.64,1)' }, onPointerDown:function(e){e.stopPropagation();} },
        R.createElement('div', { style:{ width:'100%', maxWidth:600, background:'var(--glass-bg)', backdropFilter:'blur(40px) saturate(2.5) brightness(1.15)', WebkitBackdropFilter:'blur(40px) saturate(2.5) brightness(1.15)', border:'0.5px solid var(--glass-border)', borderRadius:28, boxShadow:'0 6px 18px rgba(0,0,0,0.08), inset 0 1px 0 rgba(255,255,255,0.25)', padding:'20px 20px calc(14px + env(safe-area-inset-bottom))', overflow:'hidden', pointerEvents:'auto' } },
          ...
          currentColor && R.createElement('div', { style:{ borderTop:'0.5px solid var(--border-color)', paddingTop:7, marginTop:2 } },
            R.createElement('button', { onClick:function(){closeWithAnim(function(){onRemove && onRemove();});}, style:{ width:'100%', minHeight:46, border:'none', background:'transparent', borderRadius:16, display:'flex', alignItems:'center', justifyContent:'center', gap:8, padding:'8px 0', color:'#ff3b30', fontSize:14, fontFamily:'inherit', cursor:'pointer' } },
